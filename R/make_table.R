...
eval_estim <- function(n_rvalues, monte_carlo_iterations, theta, hist=F){
  ...
  betas <- theta[2:length(theta)]
  mds <- exp(covariables%*% betas)
  print("Mean value generated by regression: ")
  print(mean(mds))
  ...
  if(hist){
    ...
    hist(estims[, 1], main = "lambda hat", xlab = "", ylab = "")
...
ci <- function(n_rvalues, theta, alpha, monte_carlo = F, monte_carlo_hist = FALSE) {
  covariables <- matrix(rep(1, n_rvalues))
  for(i in 2:(length(theta) - 1)){
    covariables <- cbind(covariables, matrix(stats::runif(n_rvalues)))
  }
...
eval_ci <- function(n_rvalues, monte_carlo_iterations, theta, alpha, hist = F, bounds = F) {
  covariables <- matrix(rep(1, n_rvalues))
  for(i in 2:(length(theta) - 1)){
    covariables <- cbind(covariables, matrix(stats::runif(n_rvalues)))
  }
...
make_table <- function(n, theta){
  ...
  cenario <- cbind(estimacoes, taxa_de_cobertura)
  colnames(cenario) <- c("Média", "Viés", "Erro padrão", "EQM", "TC")
